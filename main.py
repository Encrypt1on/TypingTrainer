...
class TypingTrainer:
    ...
    def time_up(self):
        if self.input_entry.get() != self.generated_tests[self.curLvl][self.curTest - 1]:
            messagebox.showwarning(
                "Время истекло",
                "Вы не успели завершить тест! Начните уровень заново."
            )
            self.curTest = 1
            self.start_level()
    ...
    def check_input(self, event):
        ...
        correct_chars = len(expected_text) - Levenshtein.distance(expected_text, entered_text)
        total_chars = len(expected_text)
        accuracy = correct_chars / total_chars * 100

        if entered_text == expected_text:
            sorted_chars = (
                [c for c, _ in sorted(self.char_timings, key=lambda x: x[1])]
                if self.char_timings else []
            )
            ...
            messagebox.showerror(
                "Ошибка",
                (
                    f"Введённый текст не совпадает с заданием!\n"
                    f"Точность: {accuracy:.2f}%\n"
                    f"Скорость: {cpm:.2f} символов/мин"
                )
            )
            ...
            if self.curLvl < len(self.levels):
                ...
                if self.curTest - 1 < len(self.generated_tests[self.curLvl]):
                    ...
                    self.text_label.config(
                        text=(
                            f"Уровень {self.curLvl + 1} (Тест {self.curTest}/{self.countLvl}): "
                            f"{curTest_text}"
                        )
                    )
    ...
    def track_char(self, event):
        ...
        pause = current_time - self.char_timings[-1][1] if self.char_timings else 0
